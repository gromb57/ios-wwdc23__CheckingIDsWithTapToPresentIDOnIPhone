...
final class ContentViewModel: ObservableObject {
    ...
    func verifyButtonTapped() {
        Task {
            ...
            guard MobileDocumentReader.isSupported else {
                // This device doesn't support Tap to Present ID on iPhone.
                return
            }
    ...
    private func verifyIdentity() async throws -> MobileDriversLicenseDataRequest.Response {
        ...
        // Then, send the reader instance identifier to your server in exchange for a merchant token.
        ...
        // Your app can also cache the merchant token string after a successful `prepare` call to support offline reading.
